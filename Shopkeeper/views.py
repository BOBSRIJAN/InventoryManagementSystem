...
import os
from Shopkeeper.models import Item
...
def Shopkeeper(request):
    ...
    return render(request, 'Shopkeeper/ShopkeeperDashboard.html', {'user': user})
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            if search_term:
                ...
                return render(request, 'Shopkeeper/Manage_items.html', {
                    "items": items,
...
def Inventory_reports(request):
    ...
    return render(request, "Shopkeeper/InventoryReports.html", {
...
def Market_places(request):
    ...
    if search_term:
        if search_by == "name":
            ...
            items = Item.objects.filter(userid=user_id)
        return render(request, "Shopkeeper/MarketPlaces.html", {'items': items})
    
    items = Item.objects.filter(userid=user_id, isExpired=False)
    Market_places = items.filter(isInMarketPlaces=True, userid=user_id)
    return render(request, "Shopkeeper/MarketPlaces.html", {'items': items, 'Market_places': Market_places})
...
def Market_places_send_items(request, id):
    ...
    return render(request, 'Shopkeeper/Update.html', {'items': update})
